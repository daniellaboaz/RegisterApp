...
try {
   // const result = await sendQuery(`SELECT * FROM login WHERE email='${email}'`);
    const result = await sendQuery("SELECT * FROM login WHERE email = ?", [email]);
    ...
    //await sendQuery(`UPDATE login SET token='${resetToken}' WHERE email='${email}'`);
    await sendQuery("UPDATE login SET token = ? WHERE email = ?", [resetToken, email]);
