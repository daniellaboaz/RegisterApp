...
showusers2Router.post("/showusers2", async (req, res) => {
  try {
    ...
    if (email && name) {
      ...
      //const result = await sendQueryCommit(`SELECT username, email FROM login WHERE email = ? AND username = ?`,[email,name]);
      const result = await sendQueryCommit(
        `SELECT username, email FROM login WHERE email = ? AND username = ?`,
        email,
        name
      );
      ...
      const allUsers = await sendQueryCommit(`SELECT username, email FROM login`);
