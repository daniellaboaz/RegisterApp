...
changepassRouter.post("/changepass", async (req, res) => {
  try {
    ...
    const result = await sendQuery("SELECT password, prepass1, prepass2, prepass3 FROM login WHERE email = ?", [email]);
    if (result.length === 0) {
            ...
            await comparePassword(result[0].prepass2, newPass)){
                ...
                hashPassword(newPass.toString()).then(async(newPasswordhash) => {
                    ...
                    await sendQueryCommit2(
                        `UPDATE login SET password=?, prepass1=?, prepass2=?, prepass3=? WHERE email=?`,
                        [newPasswordhash, result[0].password, result[0].prepass1, result[0].prepass2, email]
                      ); 
