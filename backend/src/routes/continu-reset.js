...
continuresettRouter.post('/continu-reset', async (req, res) => {
  try {
    ...
    const result = await sendQuery("SELECT password, prepass1, prepass2, prepass3, token FROM login WHERE email = ?", [email]);
    ...
    await sendQueryCommit2(
      ...
      [newPasswordHash, currentPassword, prepass1, prepass2, email]
    );
